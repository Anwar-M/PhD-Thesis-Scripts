function CoherencyBeamforming2
clearvars; close all;
save_data = 1;
...
plot_settings_font(gca, '$x$ [m]', '$y$ [m]', ['Incoherent, $N_{\mathrm{B}} = 1000$'], ...
    ...
    'on', 'on', 1, [1 (round(maxvalin)-dynamic_range) round(maxvalin)], '$L_{\mathrm{p}}$ [dB]', save_data, [save_path '\IncLowFreq']);
...
plot_settings_font(gca, '$x$ [m]', '$y$ [m]', ['Coherent, $N_{\mathrm{B}} = 1000$'], ...
    [xmin xmax], [ymin ymax], linspace(xmin, xmax, 5), linspace(ymin, ymax, 5), 16, ...
    'on', 'on', 1, [1 (round(maxvalc)-dynamic_range) round(maxvalc)], '$L_{\mathrm{p}}$ [dB]', save_data, [save_path '\CohLowFreq']);
