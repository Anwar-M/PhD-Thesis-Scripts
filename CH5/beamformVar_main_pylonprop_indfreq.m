function beamformVar_main_pylonprop_indfreq
% close all;
save_data = 1;
...
[x, z, AxzCB] = FastBeamforming3Conv(CSM, scan_plane_Y , freqs, ...
        [xmin xmax zmin zmax], reso, [mic_config(:,1).';mic_config(:,3).';mic_config(:,2).'], c, [60 0 0]*(8.35-4.6)/8.35);

[x, z, AxzSC] = CleanSCConv(CSM, scan_plane_Y , freqs, ...
         [xmin xmax zmin zmax], reso, [mic_config(:,1).';mic_config(:,3).';mic_config(:,2).'], c, [60 0 0]*(8.35-4.6)/8.35);
...
SPLxzCB = 20*log10( sqrt(real(AxzCB)) / 2e-5);
SPLxzSC = 20*log10( sqrt(real(AxzSC)) / 2e-5);
SPLxzHR = 20*log10( sqrt(real(AxzHR)) / (4*pi*2e-5*8.35));

if save_data
    file_data = [fileender '.mat'];
    save([save_path '\DATA\' file_data], 'Fs', 'c', 'reso', 'xmin', 'xmax', 'zmin', 'zmax', ...
        'CSM', 'freqs', 'AxzCB', 'AxzSC', 'AxzHR', 'scan_plane_Y', 'mic_config', 'flb', 'fub', 'Tblock', 't_start', 't_end', ...
        'x', 'z', 'SPLxzCB', 'SPLxzHR', 'SPLxzSC');
end
...
figure;
contourf(x, z, SPLxzCB, (round(maxvalxz)-dynamic_range):0.5:round(maxvalxz));
...
plot_settings_font(gca, '$x$ [m]', '$z$ [m]', [strtit], ...
    ...
    [save_path '\BFCBxz' fileender '_y=' num2str(scan_plane_Y)]);
close;
