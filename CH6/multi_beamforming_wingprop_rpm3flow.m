function multi_beamforming_wingprop_rpm3flow
% close all;
save_data = 1;
remdiag = 1;
...
file_path1 = ['O:\V-Tunnel 22-05-2018 Wing-Prop Wind\Data MAT\Meas6.mat'];
file_path2 = ['O:\V-Tunnel 22-05-2018 Wing-Prop Wind\Data MAT\Meas5.mat'];
save_path = [];
...
figure;
contourf(x, y, SPL, 48:1:66);
layoutWingprop(xs, wing);

plot_settings_font(gca, '$x$ [m]', '$y$ [m]', ['Flow'], ...
    [xmin xmax], [ymin ymax], [-0.8 -0.4 0 0.4 0.8], [-0.9 -0.6 -0.3 0 0.3 0.6], ...
    16, 'on', 'on', 1, [1 48:3:66], ...
    '$L_{\mathrm{p}}$ [dB]', save_data, [mainresultpath '\CBFLOW_1-25KHZ_PROPWINGRPM3']);
...
plot_settings_font(gca, '$x$ [m]', '$y$ [m]', ['Flow'], ...
    [xmin xmax], [ymin ymax], [-0.8 -0.4 0 0.4 0.8], [-0.9 -0.6 -0.3 0 0.3 0.6], ...
    16, 'on', 'on', 1, [1 48:3:66], ...
    '$L_{\mathrm{p}}$ [dB]', save_data, [mainresultpath '\CBFLOW_1-25KHZ_PROPRPM3']);
