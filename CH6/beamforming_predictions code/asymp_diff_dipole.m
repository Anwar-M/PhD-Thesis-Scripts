function [I_s,I_end] = asymp_diff_dipole(tt,pt2,N_points,P_obs2,n,k_number,no_P_obs)
...
for i=1:N_points-1
	...
	cos_theta_s=acos( (a(i,3) + s_s(i)*e(i,3) )/ rho_s(i));
	
	factor_s = sin(cos_theta_s)*k_number^2/(4*pi);
	
	
	f_s(i) =  (factor_s)* (A(i)/((r_s(i)*rho_s(i))*(rho_r_s(i) + rho_s(i)*r_s(i))));
	...
	cos_theta_a=acos( (a(i,3) + sa(i)*e(i,3) )/ rho_a(i) );
	
	factor_a = sin(cos_theta_a)*k_number^2/(4*pi);
			
	f_a(i) = factor_a*A(i)/((ra(i)*rho_a(i))*(rho_a_ra(i)+rho_a(i)*ra(i)));
	...
	cos_theta_b=acos( (a(i,3) + sb(i)*e(i,3) ) / rho_b(i));
	
	factor_b = sin(cos_theta_b)*k_number^2/(4*pi);
	
	f_b(i) = factor_b*A(i)/((rb(i)*rho_b(i))*(rho_b_rb(i)+rho_b(i)*rb(i)));
